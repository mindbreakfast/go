...
let userClickStats = {};
let userHiddenCasinos = []; //  скрытые казино пользователя
let userViewMode = 'full'; //  режим просмотра
...
async function loadInitialData() {
    try {
        ...
        const [casinosData, userData] = await Promise.all([
            fetch('https://go-5zty.onrender.com/api/all-data').then(r => r.json()),
            fetch('https://go-5zty.onrender.com/api/user-data?userId=' + (tg?.initDataUnsafe?.user?.id || 'anonymous')).then(r => r.json()).catch(e => ({}))
        ]);
        ...
        renderCasinos();
        updateLiveRooms(); // Новое: обновляем блок с комнатами

                // Показываем стрим и анонсы
                if (dynamicData.streamStatus && dynamicData.streamStatus.isStreamLive) {
                    showStreamBanner(dynamicData.streamStatus);
                }
                if (dynamicData.announcements && dynamicData.announcements.length > 0) {
                    showAnnouncements(dynamicData.announcements);
                }

                // Отправляем статистику открытия приложения
                if (tg && tg.initDataUnsafe && tg.initDataUnsafe.user) {
                    const user = tg.initDataUnsafe.user;
        ...
        function filterCasinos() {
            return allCasinos.filter(casino => {
                ...
                return matchesSearch && matchesCategory && casino.isActive;
            });
        }
        ...
        function renderCasinos() {
            ...
            container.innerHTML = sortedCasinos.map(casino => `
                <div class="casino-card" data-id="${casino.id}">
                    ...
                    <div class="action-buttons">
                        <button class="btn btn-primary" onclick="openCasino(${casino.id})">
                            ПЕРЕЙТИ
                        </button>
        ...
        function showStreamBanner(status) {
            ...
            if (banner && streamLink && status.streamUrl) {
                ...
                // Обработчик для открытия без закрытия приложения
...
function cancelHideCasino(casinoId) {
    ...
    renderCasinos(); // Перерисовываем карточку
}
...
function requestApproval() {
    if (tg) {
        tg.openTelegramLink('https://t.me/LudogolikClickBot?start=approval');
    } else {
...
// ===== ОБНОВЛЕННЫЙ RENDER CASINOS =====
function renderCasinos() {
    const container = document.getElementById('casinoList');
    if (!container) return;

    let filteredCasinos = allCasinos.filter(casino => {
        const matchesSearch = currentSearchQuery === '' || 
            casino.name.toLowerCase().includes(currentSearchQuery) ||
            (casino.hiddenKeywords && casino.hiddenKeywords.some(kw => 
                kw.toLowerCase().includes(currentSearchQuery)
    ...
    // Добавляем кнопку показа скрытых, если есть скрытые казино
...
function openCasino(casinoId, viewMode) {
    ...
    // В компактном режиме не копируем промокод и закрываем веб-приложение
    if (viewMode === 'compact') {
        if (tg) {
            tg.openLink(casino.url);
            tg.close(); // Закрываем веб-приложение после перехода
        } else {
            window.open(casino.url, '_blank');
        }
    } else {
        // В полном режиме работаем как раньше
        copyPromoCode(casinoId, casino.promocode);
        if (tg) {
            tg.openLink(casino.url);
        } else {
